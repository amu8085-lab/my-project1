...
print(f"DEBUG: Processing {len(scenes_data)} scenes - Final Production Build.")

FALLBACK_KEYWORDS = ["deep space universe", "galaxy stars", "milky way night sky", "nebula animation"]

# Fetch Pexels with Retry and Random Page
...
# ADD BGM, WATERMARK, NORMALIZATION
...
requests.post(f"https://api.telegram.org/bot8908652813:AAFsVizGGidc-SwVGN2azUr2mgNqA9Civ34/sendMessage", json={"chat_id": chat_id, "text": f"READY_TO_UPLOAD|{video_link}|{title.replace('|', '')}|{thumbnail_prompt.replace('|', '')}|{description.replace('|', '')}"})
