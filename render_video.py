...
print(f"Total Scenes in Data: {len(scenes_data)}")
...
for i, scene in enumerate(scenes_data):
    ...
    try:
        ...
        if vid_url:
            ...
            zoomed = clip.resize(1.03).set_position(('center', 'center'))
...
final_video.set_audio(final_audio).write_videofile("final_video.mp4", fps=24, codec="libx264", audio_codec="aac", bitrate="2000k", preset="ultrafast")

# ==========================================
# UPLOAD
# ==========================================
video_link = "Upload Failed"
for name, url, field in [("0x0.st", "https://0x0.st", "file"), ("Uguu.se", "https://uguu.se/upload.php", "files[]")]:
    try:
        res = requests.post(url, files={field: open("final_video.mp4", 'rb')}, timeout=600)
        if res.status_code == 200:
            if "uguu" in name: video_link = res.json()['files'][0]['url']
            else: video_link = res.text.strip()
            break
    except: continue
